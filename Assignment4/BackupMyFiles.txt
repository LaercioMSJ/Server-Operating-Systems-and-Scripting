...
#$Destination="C:\TempBkp\"
$Staging="$Destination\Staging"
$ClearStaging=$true # When $true, Staging Dir will be cleared
#$Versions="5" #How many of the last Backups you want to keep
#$BackupDirs="C:\Temp2\", "C:\Temp\" #What Folders you want to backup
#$ExcludeDirs="C:\temp\DontBackThisUp","C:\temp\BackThisUp"#,
#,"C:\Users\seimi\OneDrive - Seidl Michael\0-Temp\Dir2" #This list of Directories will not be copied
...
{
 $Temp="^"+$Entry.Replace("\","\\")#    $Temp=$Entry.Replace("\","\\")#
    $ExcludeString+=$Temp+"|"
}
...
#Delete Zip
Function Delete-Zip {
    $Zip=Get-ChildItem $Destination | where {$_.Extension -eq ".zip"} |  Sort-Object -Property CreationTime -Descending:$false |  Select-Object -First 1
...
Function Make-Backup {
    ...
    foreach ($Backup in $BackupDirs) {
        ...
        #$fs = Get-ChildItem $Backup -Recurse  #| select * | Where-Object {$_.mode -notmatch "h" -and $_.fullname -notmatch $exclude} | select fullname #$_.mode -notmatch "h" -and 
        $Files =  Get-ChildItem $Backup -Recurse  |  Where-Object { ( $_.fullname -NOTmatch $ExcludeString)}| select fullname 
        
        foreach ($File in $Files) {
            $restpath = $file.fullname.replace($SplitBackup,"")
            try {

                #$Flder=$Backupdir+ [System.IO.Path]::GetDirectoryName($Backup)+$restpath#Backupdir
                #Copy-Item  $file.fullname $($Flder) -Force -ErrorAction SilentlyContinue |Out-Null
               Copy-Item  $file.fullname $($pth +$restpath) -Force -ErrorAction SilentlyContinue |Out-Null
...
if ($count -ge $Versions) 
{
    Delete-Backupdir
}

$CountZip=(Get-ChildItem $Destination | where {$_.Extension -eq ".zip"}).count
Logging "INFO" "Check if there are more than $Versions Zip in the Backupdir"

if ($CountZip -ge $Versions) {
    Logging "Chegou na linha 208"
    Delete-Zip 
...
if ($CheckDir -eq $false) {
    ...
    if ($Zip)
    {
        ...
        $r=$Destination+("\"+$Backupdir.Replace($Destination,'').Replace('\','')+".zip")
        $z=$Destination+("\"+$Backupdir.Replace($Staging,'').Replace('\','')+".zip")
       
        Compress-Archive -Path $Backupdir -DestinationPath $z -CompressionLevel Optimal -Force
        if ($ClearStaging)
        {
        Logging "INFO" "Clear Staging"
        Get-ChildItem -Path $Staging -Recurse -Force | remove-item -Confirm:$false -Recurse
